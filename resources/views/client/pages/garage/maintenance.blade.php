...
<div class="container small">
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-3 p-md-4">
                            <div class="d-flex justify-content-between flex-wrap gap-2 mb-2">
                                ...
                                <div>
                                    <p class="mb-0 text-muted">Mileage at Service</p>
                                    <h6 class="fw-bold mb-0">{{ $log->mileage ? number_format($log->mileage).' km' : 'N/A' }}</h6>
                                </div>
                                <div>
                                    <p class="mb-0 text-muted">Service Date</p>
                                    <h6 class="fw-bold mb-0">{{ $log->service_date ? \Carbon\Carbon::parse($log->service_date)->format('M d, Y') : 'N/A' }}</h6>
                                </div>
                            </div>
                            <div class="d-flex justify-content-start flex-wrap gap-4 mb-3">
                                <div>
                                    <p class="mb-0 text-muted">Next Due Mileage</p>
                            ...
                            <div class="mb-1">
                                <p class="mb-1 fw-bold">Maintenance Insights</p>
                                @if(is_array($log->ai_reasoning))
                                    @foreach($log->ai_reasoning as $reason)
                                        <p class="mb-1">{{ $reason }}</p>
                                    @endforeach
                                @else
                                    <p class="mb-1 text-muted">{{ $log->ai_reasoning ?? 'No AI reasoning available.' }}</p>
                                @endif
                            </div>

                            @if(!empty($log->remarks))
                                <div class="mt-2">
                                    <p class="mb-1 fw-medium">Shop Remarks</p>
                                    <p class="mb-0 text-secondary">{{ $log->remarks }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="card-body p-4">
                <div class="row gy-3">
                    <div class="col-md-6 mb-3">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <p class="mb-0 text-muted">Service Type</p>
                                <h6 class="fw-bold mb-0">{{ $latestMaintenance->service->name }}</h6>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 mb-3">
                ...
                <div class="mt-4">
                    ...
                    @if(is_array($latestMaintenance->ai_reasoning))
                            ...
                            @endforeach
                    @else
                        <p class="text-muted">{{ $latestMaintenance->ai_reasoning ?? 'No insights available.' }}</p>
                    @endif
                </div>
            </div>
        </div>
    @else
        <div class="text-center py-5 bg-white">
            ...
            <p class="text-secondary">You haven’t logged any maintenance yet. Add one to get AI-based insights and reminders.</p>
        </div>
    @endif
</div>
@endsection
