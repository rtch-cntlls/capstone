<div class="p-2 mt-3">
    <div class="d-flex justify-content-between align-items-center">
        <h4 class="fw-bold m-0">Service Management</h4>
        <div class="d-flex ">
            <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#createServiceModal">
                ...
                <ul class="dropdown-menu dropdown-menu-end shadow-sm" aria-labelledby="exportDropdown" style="font-size: 13px;">
                    <li>
                        <a class="dropdown-item d-flex align-items-center" 
                           href="{{ route('admin.services.export.pdf')}}">
                            <i class="fas fa-file-pdf text-danger me-2"></i> Export as PDF
                        </a>
                    </li>
                    <li>
                        <a class="dropdown-item d-flex align-items-center" 
                           href="{{ route('admin.services.export.csv') }}">
...
<div>
    <div class="row row-cols-2 g-3 p-2 mb-2">
        @foreach ($cards as $card)
            <div class="col">
                @php
                    $barColor = $card['value'] > 0 ? 'bg-success' : 'bg-secondary';
                @endphp
                <a href="{{ route('admin.services.index', ['status' => $card['title']]) }}" class="text-decoration-none">
                    <div class="card h-100 hover-shadow">
                        <div class="card-body">
                            <div class="card-title mb-2 small text-muted text-start mb-4">
                                <i class="{{ $card['icon'] }} me-1"></i>
                                <span>{{ $card['title'] }} Services</span>
                            </div>
