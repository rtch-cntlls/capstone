<div class="p-2 mt-3">
    <div class="d-flex justify-content-between align-items-center">
        <h4 class="fw-bold m-0">Booking Summary</h4>
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow-sm" aria-labelledby="exportDropdown" style="font-size: 13px;">
                <li>
                    <a class="dropdown-item d-flex align-items-center" 
                       href="{{ route('admin.bookings.export.pdf', $status ? ['status' => $status] : []) }}">
                        <i class="fas fa-file-pdf text-danger me-2"></i> Export as PDF
                    </a>
                </li>
                <li>
                    <a class="dropdown-item d-flex align-items-center" 
                       href="{{ route('admin.bookings.export.csv', $status ? ['status' => $status] : []) }}">
...
<div>
    <div class="row row-cols-2 row-cols-sm-2 row-cols-md-4 g-3 p-2 mb-2">
        @foreach ($cards as $card)
            <div class="col">
                @php
                    $statusParam = $card['title'] === 'Pending/Approved' ? null : $card['title'];
                @endphp
                <a href="{{ route('admin.bookings.index', $statusParam ? ['status' => $statusParam] : []) }}" class="text-decoration-none">
